...
// Builds a compact snapshot of the CURRENT user's tasks, notes, and
// calendar events (scoped by ownerEmail). This snapshot is injected
// into the system prompt so the AI can reference existing MongoDB _id
// values when the user asks it to update, toggle, or delete something
// — and never sees another account's data.
//
...
async function buildSnapshot(ownerEmail) {
  ...
  // ----- Tasks -----
  try {
    const tasks = await TaskService.findAll("all", ownerEmail);
  ...
  // ----- Calendar events -----
  try {
    const events = await CalendarService.findAll(ownerEmail);
